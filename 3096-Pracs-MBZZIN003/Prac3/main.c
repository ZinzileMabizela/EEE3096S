...
int main(void)
{
  ...
  while (1)
  {
	...
	uint16_t adcOut = pollADC();
	char output[4];
	sprintf(output, "%d", adcOut);// converts adcValue to output char Array
	writeLCD(output);
...
void EXTI0_1_IRQHandler(void)
{
	...
	if((curr_millis - prev_millis) >= delay_t){// preventing mechanical bouncing
		if( delay_t == 500){
			delay_t = 1000;
		}else if(delay_t == 1000){
			delay_t = 500;
		}// If Statement Changing the time to keep the LED off
		prev_millis = HAL_GetTick();// getting the System Counter Value after switch
	}// end of debouncing
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	HAL_ADC_PollForConversion(&hadc, 3000); // Waiting for end of conversion
	val = HAL_ADC_GetValue(&hadc);// receives the converted value from register
	// end of polling

	HAL_ADC_Stop(&hadc);//stops ADC conversions
	return val;
}
