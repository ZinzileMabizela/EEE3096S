...
//
static uint16_t countAddress = 0;

static uint8_t patternsArray[]={
	0b10101010,
	0b01010101,
	0b11001100,
	0b00110011,
...
int main(void)
{
  ...
  for (uint16_t i = 0; i < sizeof(patternsArray); i++) {
  		write_to_address(i, patternsArray[i]);
  	}

  uint32_t fullPeriod = 1000;
  uint32_t halfPeriod = 500;
  uint32_t currentPeriod = fullPeriod;
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0) {
		  if(currentPeriod == fullPeriod) {
		  			  currentPeriod = halfPeriod;
		  }
		  else if (currentPeriod == halfPeriod) {
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t temp = read_from_address(countAddress);

	if (temp==patternsArray[countAddress])
		{
			LL_GPIO_ResetOutputPin(GPIOB, 0b11111111);
		...
		{
			LL_GPIO_ResetOutputPin(GPIOB, 0b11111111);
			LL_GPIO_TogglePin(GPIOB, 0b00000001);
		}
